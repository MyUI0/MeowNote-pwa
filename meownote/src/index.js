...
<meta name="apple-mobile-web-app-title" content="MeowNote">
<meta name="mobile-web-app-capable" content="yes">
<meta name="theme-color" content="#6366f1">
<script src="/pwa.js"></script>
<link rel="manifest" href="/manifest.json">
<link rel="apple-touch-icon" href="https://img.icons8.com/color/180/cat.png">
...
body{font-family:var(--font);background:var(--bg);color:var(--text);font-size:15px;line-height:1.6;-webkit-font-smoothing:antialiased;transition:background .2s,color .2s,background-image .3s;padding-top:env(safe-area-inset-top);min-height:calc(100vh + env(safe-area-inset-top) + env(safe-area-inset-bottom));position:relative}
body::before{content:'';position:absolute;top:calc(-1 * env(safe-area-inset-top));left:0;right:0;height:calc(env(safe-area-inset-top) + env(safe-area-inset-bottom));background:inherit;z-index:-1}
...
<div class="overlay" id="settings-overlay">
  <div class="modal" style="width:500px;max-height:88vh;overflow-y:auto">
    ...
    <!-- ========== 壁纸设置 ========== -->
    <div style="font-size:11px;font-weight:700;letter-spacing:.08em;color:var(--text3);text-transform:uppercase;margin:18px 0 10px">🖼️ 壁纸设置</div>
    <div style="display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;margin-bottom:10px">
      <button class="btn btn-primary" id="wp-random-btn" onclick="selectWallpaper('random',this)" style="font-size:13px;padding:8px">🎲 随机</button>
      <button class="btn btn-ghost" id="wp-fixed-btn" onclick="selectWallpaper('fixed',this)" style="font-size:13px;padding:8px">📌 固定</button>
      <button class="btn btn-ghost" id="wp-none-btn" onclick="selectWallpaper('none',this)" style="font-size:13px;padding:8px">❌ 无</button>
    </div>
    
    <div id="fixed-wp-panel" style="display:none;background:var(--surface2);border-radius:8px;padding:10px;margin-bottom:12px">
      <label style="font-size:12px;color:var(--text3);display:block;margin-bottom:6px">固定壁纸图片 URL</label>
      <div style="display:flex;gap:6px">
        <input type="text" id="fixed-wp-url" placeholder="https://example.com/wallpaper.jpg" 
          style="flex:1;font-size:12px;padding:6px 8px;border:1px solid var(--border);border-radius:6px;background:var(--surface);color:var(--text)"
          onkeydown="if(event.key==='Enter')saveFixedWallpaper()"/>
        <button class="btn btn-primary btn-sm" onclick="saveFixedWallpaper()" style="font-size:12px;padding:6px 12px">保存</button>
      </div>
      <p style="font-size:11px;color:var(--text3);margin-top:6px">💡 输入图片链接后点击保存</p>
    </div>
    
    <div id="random-wp-tip" style="font-size:11px;color:var(--text3);padding:8px;background:var(--surface2);border-radius:6px;margin-bottom:12px">
      🔄 每次刷新页面自动更换随机动漫壁纸（来源：dmoe.cc）
    </div>
    <!-- ========== 壁纸设置结束 ========== -->
...
// ========== 壁纸选择函数 ==========
function selectWallpaper(type,btn){
  ['wp-random-btn','wp-fixed-btn','wp-none-btn'].forEach(id=>{
    const el=document.getElementById(id);
    if(el){el.className='btn btn-ghost';el.style.fontSize='13px';el.style.padding='8px';}
  });
  ...
  if(window.applyWallpaper)window.applyWallpaper(type);
  
  const fixedPanel=document.getElementById('fixed-wp-panel');
  const randomTip=document.getElementById('random-wp-tip');
  
  if(type==='fixed'){
    fixedPanel.style.display='block';
    randomTip.style.display='none';
    const savedUrl=localStorage.getItem('mn_fixed_bg');
    if(savedUrl&&document.getElementById('fixed-wp-url')){
      document.getElementById('fixed-wp-url').value=savedUrl;
    }
  }else if(type==='random'){
    fixedPanel.style.display='none';
    randomTip.style.display='block';
  }else{
    fixedPanel.style.display='none';
    randomTip.style.display='none';
  }
...
function saveFixedWallpaper(){
  const urlInp=document.getElementById('fixed-wp-url');
  const url=urlInp.value.trim();
  
  if(!url){alert('请输入图片 URL');return;}
  
  try{new URL(url);}catch(e){alert('请输入有效的图片 URL');return;}
  
  if(window.setFixedWallpaper)setFixedWallpaper(url);
  toast('固定壁纸已保存 ✓');
  
  const fixedBtn=document.getElementById('wp-fixed-btn');
  if(fixedBtn)selectWallpaper('fixed',fixedBtn);
}
// ========== 壁纸选择函数结束 ==========
